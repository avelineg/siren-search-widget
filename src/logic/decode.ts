...
const trancheEffectifsMap: Record<string, string> = {
...
/**
 * Renvoie le libellé correspondant à la tranche d'effectifs
 */
export function decodeTrancheEffectifs(code: string) {
  if (!code) return "–"
  return trancheEffectifsMap[code] ?? code
}
